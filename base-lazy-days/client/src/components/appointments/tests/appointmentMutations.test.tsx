...
test("Cancel appointment", async () => {
  ...
  // check for the toast alert
  const alertToastCancel = await screen.findByRole("alert");
  expect(alertToastCancel).toHaveTextContent("Your appointment cancelled");

  // close alert to keep state clean and wait for it to disappear
  const alertCloseButtonCancel = screen.getByRole("button", { name: "Close" });
  alertCloseButtonCancel.click();
